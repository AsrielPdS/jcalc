...
function exp(value: exp.Expression, options?: exp.ParseOptions) {
...
namespace exp {
  ...
  export class SumOp extends OpVal {
    ...
    calc(opts: CalcOptions) {
      let
        a = this[0].calc(opts) as number | string,
  ...
  export class ObjectVal implements IValue {
    ...
    calc(opts: CalcOptions) {
      let
        i = 1, l = this.levels,
        result = typeof opts.vars == 'function' ? <any>opts.vars(l[0], true) : opts.vars[l[0]];
      for (; i < l.length; i++) {
        if (!result) {
          return null;
        }
  ...
  var translate: (funcName: string, signal: TranslateDir) => string;
  export function defineTranslateFunc(func: typeof translate) {
    translate = func;
  }
  ...
  export function calc(exp: Expression, options: CalcOptions) {
    if (!exp) return null;
  ...
  export function compileExpression(expression: string): IValue {
    throw "unsetted797";
  }
